...
if(isset($_POST['upload-base_tango'])){
    ...
    if (is_uploaded_file($tmpEXCEL)){
        ...
        $spread = IOFactory::load('cargue/Cargar_Tango.xls');
        //echo 'Hola Excel';
        ...
        for ($i = 2; $i <= $nomRows; $i++){
            ...
            $NUMERO_ORDEN= $spread->getActiveSheet()->getCell('A'.$i)->getCalculatedValue();
            $LOGICA=$spread->getActiveSheet()->getCell('B' .$i)->getFormattedValue();
            $FECHA_ASIGNACION= $spread->getActiveSheet()->getCell('C' .$i)->getFormattedValue();
            $FECHA_COMPROMISO= $spread->getActiveSheet()->getCell('D' .$i)->getFormattedValue();
            $HORA= $spread->getActiveSheet()->getCell('E' .$i)->getFormattedValue();
            $RUT= $spread->getActiveSheet()->getCell('F' .$i)->getFormattedValue();
            $CIUDAD_LOCALIDAD= $spread->getActiveSheet()->getCell('G' .$i)->getCalculatedValue();
            $CNC= $spread->getActiveSheet()->getCell('H' .$i)->getFormattedValue();
            $PUESTO_TABAJO= $spread->getActiveSheet()->getCell('I' .$i)->getCalculatedValue();
            $PLATAFORMA= $spread->getActiveSheet()->getCell('J' .$i)->getCalculatedValue();
